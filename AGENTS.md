...
- 决策优先级固定为：本文件的安全与执行规则 → 当前运行代码、协议和测试 → `docs/agent-workbench-v2.md` 与 `README.md` 的当前产品说明 → `DEVELOPMENT.md` 顶部最新日期条目。
- `DEVELOPMENT.md` 较早日期的内容只记录历史，不能用于推翻当前代码或当前产品说明；旧测试数量、旧页面名称、旧导航结构和旧截图均不得作为现状依据。
- 当前 Web 一级入口只有“今天、计划、我的”，顶栏快捷操作只有“记一笔”。未导航的任务、上下文、规则、实验和校准页面是兼容或诊断能力，不得据此恢复旧产品结构。
- 已删除的设计 QA、旧闭环验证矩阵和旧 dashboard 截图不是可用上下文，不得重新生成或引用，除非用户明确要求做历史考古。
...
- 默认不调用外部模型 API，不要求用户配置 API Key；仅在用户显式通过环境变量或 Web UI 的“我的 → 高级设置 → 智能规划设置”配置 provider、模型和对应 API Key 后，才允许调用用户选择的模型服务。Web UI 接入只允许把 key 放入当前服务进程环境，不得落库或写文件；DeepSeek provider 只用于文本型原材料和每日复盘，照片任务不得伪造未支持的视觉输入。
