...
- `memory`, `adjustment`
- versioned `preferences` for profile, settings, private doctrine, check-in module configuration, the compact `agent_user_model`, confirmed `goal_contract`, and current `meal_episode_projection`; detailed Agent runs and unpublished drafts remain local diagnostics
