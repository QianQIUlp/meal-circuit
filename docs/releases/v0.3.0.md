...
- Android APK and AAB artifacts are included only when the complete MealCircuit release-key configuration is available. The APK is verified with `apksigner`, and the AAB is strictly verified against the restored release keystore and its configured alias; this accepts Android's self-signed release certificate without trusting an unrelated public CA. Without the key, a tagged release still publishes its verified desktop and Linux assets but omits Android rather than publishing an unsigned APK or AAB.
