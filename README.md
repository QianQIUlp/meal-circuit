...
Built-in generation is manual and opt-in. Set `MEALCIRCUIT_AI_PROVIDER` to `openai` or `anthropic`, set `MEALCIRCUIT_AI_MODEL` to the exact model you want to pay for, and provide either `MEALCIRCUIT_OPENAI_API_KEY` or `MEALCIRCUIT_ANTHROPIC_API_KEY`. MealCircuit reads these values from the process environment only; it does not store keys in SQLite or the browser UI. You can also set `MEALCIRCUIT_AI_TIMEOUT_SECONDS` and `MEALCIRCUIT_AI_MAX_OUTPUT_TOKENS`.
...
> [!IMPORTANT]
> MealCircuit 默认不要求 API Key。它负责保存事实、组装上下文和校验结果；Codex、Claude Code 或其他 Agent 仍可负责分析。你也可以选择在环境变量中配置自己的 OpenAI 或 Anthropic API Key，并手动触发内置生成来处理待办。
...
内置生成是手动、可选能力。设置 `MEALCIRCUIT_AI_PROVIDER=openai|anthropic`，用 `MEALCIRCUIT_AI_MODEL` 明确填写你愿意付费使用的模型名，并提供 `MEALCIRCUIT_OPENAI_API_KEY` 或 `MEALCIRCUIT_ANTHROPIC_API_KEY`。MealCircuit 只从当前进程环境变量读取这些值，不写入 SQLite，也不进入浏览器页面。可选参数包括 `MEALCIRCUIT_AI_TIMEOUT_SECONDS` 和 `MEALCIRCUIT_AI_MAX_OUTPUT_TOKENS`。
