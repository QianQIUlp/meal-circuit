...
- 核心功能：把真实 instrumentation job 移到 `macos-15-intel`，使用 GitHub 托管 macOS 的硬件加速虚拟化；保留 API 35、完整 Gradle instrumentation、真实同步服务、Python → Android → 新 Python 双向验证和服务端明文扫描。写入 `GITHUB_ENV` 前使用 `add-mask` 隐藏合成恢复字符串，避免失败日志的环境摘要回显测试凭据。
