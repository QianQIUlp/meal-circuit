...
- 目标：在保留 Codex/Claude Code 外部 Agent 工作流的同时，让用户可用自己的 OpenAI 或 Anthropic API Key 手动处理待办。
...
- 供应商边界：支持 `MEALCIRCUIT_AI_PROVIDER=openai|anthropic`、显式 `MEALCIRCUIT_AI_MODEL`、对应 API Key、超时和最大输出 token；OpenAI 使用 Responses API 图片 data URL 与 JSON schema，Anthropic 使用 Messages API 图片 base64 与强制 tool result。
- 验证：`python -m py_compile mealcircuit\ai.py mealcircuit\service.py mealcircuit\server.py mealcircuit\agent_cli.py mealcircuit\configuration.py tests\test_mealcircuit.py`、41 项 `.\test.ps1` 测试、`python tools\release_check.py` 和 `git diff --check` 通过；新增覆盖缺少环境变量不写库、OpenAI 照片 payload、Anthropic tool payload、非法模型结果保持 pending、Web 成功/失败路径。
