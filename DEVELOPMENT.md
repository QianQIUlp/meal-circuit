...
- 改动文件：仅调整 `.github/workflows/test.yml` 的 instrumentation runner，并记录本轮过程；不改变 Android、同步协议、服务端或业务逻辑。
...
- 核心功能：把真实 instrumentation job 移到 `macos-15-intel`，使用 GitHub 托管 macOS 的硬件加速虚拟化；保留 API 35、完整 Gradle instrumentation、真实同步服务、Python → Android → 新 Python 双向验证和服务端明文扫描。
