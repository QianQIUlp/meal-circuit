...
- 核心改动：回执表单支持直接拍照或选择图片，照片沿用既有本地照片任务并作为这顿的真实执行证据关联；缺少变化原因时先在当前表单内提示，服务端也会原页返回并恢复实际情况、份量感觉、文字与已上传照片。补选原因后可继续提交，不会生成一条半成品回执。
- 验证：运行定向测试 `python -m unittest tests.test_mealcircuit.WebAppTest.test_plan_feedback_preserves_inputs_and_photo_after_missing_reason`，覆盖漏选原因后的 400 原页恢复、照片持久化、重复原因字段解析、补全后的成功提交和执行证据关联，结果通过；`git diff --check` 通过。按用户要求未运行浏览器校验、全量测试、Android 构建或 CI 等待。
- 用户用法：在计划的任一餐展开“吃得怎么样？”，可选一张实际照片；若选择了有变化的结果却没说明原因，页面会就地提醒，已填内容仍在，补选后再次点击“记下来”即可。
